...
public class MyBot : IChessBot {
    ...
    public Move Think(Board board, Timer timer) {
        ...
        int firstGuess = 0, depth = 0;
        // Iterative deepening loop
                for (;; depth++) {
                    // Console.WriteLine(depth);
                    int g = firstGuess, upperBound = 99999, lowerBound = -99999;

                    if (timer.MillisecondsElapsedThisTurn >= searchMaxTime)
                        return rootMove;

                    while (lowerBound < upperBound) {
                        int beta = Math.Max(g, ++lowerBound);
                        // g = AlphaBeta(beta - 1, beta, depth, 0);
                        g = PVS(depth, beta - 1, beta, 0, true);
        ...
        #if TESTING
                        Console.WriteLine("g: " + g + "   beta: " + beta + "   lower, upper: " + lowerBound + ", " + upperBound);
        #endif
                    }
        ...
        // int depth = 0;
        // for (;; depth++) {
        //     int score = -99999, min = score, max = 99999;

        //     if (timer.MillisecondsElapsedThisTurn >= searchMaxTime)
        //         return rootMove;

        //     while (min != max && score != 99999) {
        //         int alpha = (min + max) / 2;
        //         // score = failSoftAlphaBeta(alpha, alpha + 1, depth);
        //         score = PVS(depth, alpha, alpha + 1, 0, true);
        //         if (score > alpha)
        //             min = score;
        //         else
        //             max = score;
        //         Console.WriteLine("score: " + score + "   alpha: " + alpha + "   min, max: " + min + ", " + max);
        //     }
        //     // return score;
        // }
